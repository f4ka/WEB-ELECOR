	<div class="sign-up">
		<!-- container -->
		<div class="container">
			<div class="sign-up-left">
				<p><span></span></p>
			</div>
			<div class="clearfix"> </div>
		</div>
		<!-- //container -->
	</div>
	<!-- footer -->
	<div class="footer">
		<!-- container -->
		<div class="container">
			<div class="col-md-3 stores-grid">
				<div class="stores">
					<h3>Nuestra Tienda</h3>
					<ul>
                        <li>Distribuidor Autorizado Fluke-Tektronix</li>
                        <li>CP 4000 - San Miguel de Tucumán</li>
                        <li>Teléfono/Fax: 381-4330467</li>
                        <li>Celular: 381-156420351</li>

						<span>
						<li class="drive"> E-Mail: </li>
						<li>elecor@arnet.com.ar</li>
						<li>instrumentos@elecor.com.ar</li>
                        <li>mgatica@elecor.com.ar</li>
						</span>
					</ul>
				</div>
				<div class="social-icons white-icons">
					<ul>
						<li><a href="#" class="facebook"></a></li>
						<li><a href="#" class="twitter"></a></li>
						<li><a href="#" class="chrome"></a></li>
						<li><a href="#" class="vimeo"></a></li>
						<li><a href="#" class="rss"></a></li>
				</div>
			</div>
			<div class="col-md-3 blog">
				<h3>Sobre la empresa</h3>
				<p align="justify">
                    Surge  de “Electrónica Corrientes”, nace en el mes Julio  1991,
                    con el solo propósito de ofrecer a las industrias del Noa, un servicio
                    reparación de instrumentos de medición eléctrica y electrónica...
				</p>
			</div>
			<div class="col-md-3 support">
				<h3>Productos</h3>
				<div class="support-grids">
					<div class="support-left">
						<ul>
							<li><a href="http://www.fluke.com">Fluke</a></li>
							<li><a href="http://www.amprobe.com">Amprobe</a></li>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
						</ul>
					</div>
					<div class="support-left support-right">
						<ul>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
							<li><a href="#"></a></li>
						</ul>
					</div>
					<div class="clearfix"> </div>
				</div>
			</div>
			<div class="col-md-3 contact">
				<h3>Contactenos</h3>
				<form>
					<input type="text" value="Su e-mail..." onfocus="this.value = '';" onblur="if (this.value == '') {this.value = 'Su e-mail...';}" required="">
					<textarea value="Su texto...:" onfocus="if(this.value == 'Su texto...') this.value='';" onblur="if(this.value == '') this.value='Su texto...';">Su texto...</textarea>
					<input id="enviar" type="submit" value="Enviar mensaje">
				</form>
			</div>
			<div class="clearfix"> </div>
			<div class="copyright">
				<p>Template by <a href="http://w3layouts.com/">W3layouts</a></p>
			</p>
		</div>
		<!-- // container -->
	</div>
	<!-- //footer -->

		<!-- //container -->
	</div>
